...
/// Modified to detect and prevent numeric overflow
/// </summary>
...
/// <param name="result">Reference to store the computed result</param>
/// <returns>true if successful, false if overflow would occur</returns>
template <typename T>
bool add_numbers(T const& start, T const& increment, unsigned long int const& steps, T& result)
{
    ...
    {
        // Check for overflow: if (max - current) < increment, then overflow will occur
        if (std::numeric_limits<T>::max() - result < increment)
        {
            // Overflow would occur, set result to max value and return false
...
/// Modified to detect and prevent numeric underflow
/// </summary>
/// <typeparam name="T">A type that with basic math functions</typeparam>
/// <param name="start">The number to start with</param>
...
/// <param name="steps">The number of steps to iterate</param>
/// <param name="result">Reference to store the computed result</param>
/// <returns>true if successful, false if underflow would occur</returns>
template <typename T>
bool subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps, T& result)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // Check for underflow: if current < decrement, then underflow will occur
        if (result < decrement)
        {
            // Underflow would occur, set result to min value and return false
...
void test_overflow()
{
    ...
    // Call 1 (expected success)
    std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";

    T result{};
    bool ok = add_numbers<T>(start, increment, steps, result);

    std::cout << "overflow status = " << std::boolalpha << (!ok)   // overflow status true means overflow occurred
              << ", result = " << +result << std::noboolalpha << std::endl;

    // Call 2 (expected overflow)
    std::cout << "\tAdding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";

    ok = add_numbers<T>(start, increment, steps + 1, result);

    std::cout << "overflow status = " << std::boolalpha << (!ok)
              << ", result = " << +result << std::noboolalpha << std::endl;
}
...
void test_underflow()
{
  ...
  // Call 1 (expected success)
    ...
    std::cout << "underflow status = " << std::boolalpha << (!ok)
              << ", result = " << +result << std::noboolalpha << std::endl;

    // Call 2 (expected underflow for unsigned, or for some signed depending on values)
    std::cout << "\tSubtracting Numbers With Overflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = ";

    ok = subtract_numbers<T>(start, decrement, steps + 1, result);
